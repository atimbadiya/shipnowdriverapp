...
class CalenderViewController: UIViewController,FSCalendarDataSource, FSCalendarDelegate {
    ...
    var checkfordriverrating = ""


    override func viewDidLoad() {
        super.viewDidLoad()
        
        cancelbtntext.setTitle("CANCEL".localized, for: UIControl.State.normal)
        okbtntext.setTitle("OK".localized, for: UIControl.State.normal)
     ...
     /*   let date = Date()
      ...
      let components =  calendar.dateComponents([.day, .month, .year, .hour], from: date as Date)
        ...
        enddate = formatter.string(from: date as Date)
        
        startdate = formatter.string(from: date as Date)
  ...
  /*     func calendar(_ calendar: FSCalendar, didSelect date: Date) {
    ...
    @IBAction func okbtn(_ sender: Any) {
        ...
        if checkmovefrom == "uploaddocument"{
             ...
             if AppConstants.movefromcalender == 1{

               // AppConstants.insurancdocumentdate = selecteddate
                self.dismiss(animated: true, completion: nil)

            }else  if AppConstants.movefromcalender == 2{

              //  AppConstants.licensedocumentdate = selecteddate
                self.dismiss(animated: true, completion: nil)

             }else  if AppConstants.movefromcalender == 3{

               // AppConstants.rcdocumentdate = selecteddate
                 self.dismiss(animated: true, completion: nil)

             }else  if AppConstants.movefromcalender == 4{

                AppConstants.documentdateselected = selecteddate
                self.dismiss(animated: true, completion: nil)
        ...
        else if checkfrom == "superdriver"{
            
            AppConstants.checkfromsuperdriver = "1"
            AppConstants.superdateselected = startdate
            self.dismiss(animated: true, completion: nil)
            
        } else if checkfrom == "endsuperdriver"{
            
            AppConstants.endcheckfromsuperdriver = "1"
            AppConstants.endsuperdateselected = enddate
            self.dismiss(animated: true, completion: nil)
            
            
        }else if checkfordriverrating == "selectdatedriverrating"{
            
            AppConstants.selectcheck = "1"
            AppConstants.documentdateselected = selecteddate
            self.dismiss(animated: true, completion: nil)
            
        }else if checkfordriverrating == "startdriverrating"{
            AppConstants.startcheck = "1"
            AppConstants.superdateselected = startdate
            self.dismiss(animated: true, completion: nil)
            
        }else if checkfordriverrating == "enddriverrating"{
            AppConstants.endcheck = "1"
            AppConstants.endsuperdateselected = enddate
            self.dismiss(animated: true, completion: nil)
            ...
            AppConstants.checkmovefromcalenderearning = "1"
            AppConstants.dateselected = selecteddate
      //  AppConstants.checkmovefromcalenderearning = "1"
       self.dismiss(animated: true, completion: nil)
        }
