...
public class QRCodeScannerController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationBarDelegate {
    ...
    func createCornerFrame() {
        ...
        let rect = CGRect.init(
            origin: CGPoint.init(
                ...
                y: self.view.frame.midY - (height+bottomSpace)/2),
    ...
    func addMaskLayerToVideoPreviewLayerAndAddText(rect: CGRect) {
        ...
        noteText.string = "Align QR/Barcode within frame to scan"
    ...
    private func addButtons(_ view: UIView) {
        ...
        //Segmented Control button for choosing code type
        let items = ["Scan QR code", "Scan Barcode"]
        let segmentController = UISegmentedControl(items: items)
        
        if UserDefaults.standard.value(forKey: "LastScannedCodeType") != nil && UserDefaults.standard.value(forKey: "LastScannedCodeType") as! String == ".code128"
        {
        ...
        segmentController.frame = CGRect(x:frame.minX+20, y:frame.minY+15,width: frame.maxX-40,height: 30)
        segmentController.backgroundColor = UIColor.black
        segmentController.addTarget(self, action: #selector(segmentControlValueChanged(sender:)), for: .valueChanged)
        ...
        segmentController.setTitleTextAttributes(titleTextAttributesNormal, for: .normal)
        segmentController.setTitleTextAttributes(titleTextAttributesSelected, for: .selected)
    ...
    @objc func segmentControlValueChanged(sender: UISegmentedControl) {
        ...
        case 0:
            UserDefaults.standard.setValue(".qr", forKey: "LastScannedCodeType")
            captureSession.stopRunning()
            dataOutput.metadataObjectTypes = [.qr]
            print("dataOutput.metadataObjectTypes \(String(describing: dataOutput.metadataObjectTypes))")
            captureSession.startRunning()
        case 1:
            UserDefaults.standard.setValue(".code128", forKey: "LastScannedCodeType")
            captureSession.stopRunning()
            dataOutput.metadataObjectTypes = [.code128]
            print("dataOutput.metadataObjectTypes \(String(describing: dataOutput.metadataObjectTypes))")
            captureSession.startRunning()
        default:
            dataOutput.metadataObjectTypes = [.qr]
    ...
    private func setupCaptureSession(_ devicePostion: AVCaptureDevice.Position) {
        ...
        dataOutput.metadataObjectTypes = [.qr]
        if (UserDefaults.standard.value(forKey: "LastScannedCodeType") != nil)
        {
            if (UserDefaults.standard.value(forKey: "LastScannedCodeType") as! String == ".code128")
            {
                dataOutput.metadataObjectTypes = [.code128]
            }
        }
                ...
                if view.bounds.contains(unwraped.bounds) {
                    ...
                    if delCnt > delayCount {
                        if let unwrapedStringValue = unwraped.stringValue {
                            ...
                            delegate?.qrScanner(self, scanDidComplete: unwrapedStringValue, metadataType: metadataObjectType)
                        } else {
